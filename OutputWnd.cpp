...
void COutputWnd::upDateOutPutWnd(CString *inputmessage)
{
	fprintf(fp, inputmessage->GetBuffer(0));
	fprintf(fp,"\r\n");
//	m_wndOutputBuild.InsertString(0, _T(*inputmessage));
	m_wndOutputBuild.AddString(_T(*inputmessage));
	m_wndOutputBuild.SetCurSel(m_wndOutputBuild.GetCount()-1);
}


void COutputWnd::upDateOutPutWndDBLine(CString *inputmessage)
{
	fprintf(fp, inputmessage->GetBuffer(0));
	fprintf(fp,"\r\n");
	//	m_wndOutputBuild.InsertString(0, _T(*inputmessage));
	m_wndOutputDebug.AddString(_T(*inputmessage));
